...
export default function PostComponent(props) {
  ...
  useEffect(() => {
    urlMetadata(
      `localhost:5005/proxy?url=www.npmjs.com/package/react-icons`
    ).then(
      metadata => {
    ...
    axios
      .get(`https://jsonlink.io/api/extract?ur1=${props.articleUrl}`)
      .then(res => {
        parseHTML(res.data)
      })
      .catch(console.log)
  ...
  function handleEnter(e) {
    ...
    axios
      .patch(
        `${process.env.REACT_APP_API_URL}/post/${props.id}`,
        { newPost },
      ...
      .catch(err => {
        console.log(err)
        alert(
          'Houve algum erro em editar o seu post! Tente novamente ou atualize a tela!'
        )
        setDisabled(false)
      })
      .finally(() => {})
  ...
  return (
    <PostContainer>
      ...
      <div className="esquerda">
        <h2 onClick={() => navigate(`/user/${props.userId}`)}>
          {props.username}
        </h2>
        {editor ? (
        ...
        {id === props.userId ? (
          <OwnerOptions>
            <IconEdit
              form={`edit${props.id}`}
              type={editor ? 'reset' : ''}
              onClick={() => (editor ? setEditor(false) : openEditor())}
              editor={editor}
            >
